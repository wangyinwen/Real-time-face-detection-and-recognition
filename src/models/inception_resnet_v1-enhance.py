...
def reduction_a(net, k, l, m, n):#复制减半数据空间尺寸，并且加深通道
    with tf.variable_scope('Branch_0'):
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                ...
                # 5 x Inception-resnet-A--# 35 x 35 x 256
                net = slim.repeat(net, 10, block35, scale=0.17)
                end_points['Mixed_5a'] = net
        
                # Reduction-A--# 17 x 17 x 896
                with tf.variable_scope('Mixed_6a'):
                    net = reduction_a(net, 192, 192, 256, 384)
                end_points['Mixed_6a'] = net
                
                # 10 x Inception-Resnet-B--# 17 x 17 x 896
                ...
                # 5 x Inception-Resnet-C--8*8*1792
                net = slim.repeat(net, 10, block8, scale=0.20)
